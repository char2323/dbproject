...
<template>
  <div class="p-4 md:p-8">
    ...
    <div v-if="!isLoading && coupons.length > 0" class="space-y-4">
      <div v-for="coupon in coupons" :key="coupon.id" class="p-4 bg-white rounded-lg shadow-md border-l-4 border-yellow-400">
        <div class="flex justify-between items-center">
          ...
          <div class="text-right">
            ...
            <p class="text-sm text-red-500">优惠券</p>
          </div>
        </div>
      </div>
    </div>

    <div v-if="!isLoading && coupons.length === 0" class="text-center text-gray-500 mt-8">
      <p>您当前没有任何优惠券</p>
    </div>
  </div>
</template>
